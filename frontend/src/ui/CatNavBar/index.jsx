...
export default function CatNavBar({data}) {
    ...
    return (
        <nav ref={navRef} className='flex w-full mobile:px-10 h-12 mobile:h-10 items-center justify-center bg-secondaryBackground overflow-hidden flex-row flex-nowrap'>
            <div className="absolute left-0 px-1 py-catNav bg-secondaryBackground hidden mobile:block shadow-catRight hover:bg-navigation hover:text-white cursor-pointer" onClick={scrollLeft}>
            ...
            {data.map(el =>
                ...
                <Bouton intent="navigation" size="catNavigation" className='uppercase hover:bg-navigation'>{el.name}</Bouton>
                </NavLink>
            )}
            <div className="absolute right-0 px-1 py-catNav bg-secondaryBackground hidden mobile:block shadow-catLeft hover:bg-background hover:text-white cursor-pointer" onClick={scrollRight}>
